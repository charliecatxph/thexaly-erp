...
export const sphereTreeSlice = createSlice({
    ...
    reducers: {
        ...
        setNaviShown: (state, action: { payload: boolean }) => {
            state.naviShown = action.payload
        },
        setNaviMount: (state, action: { payload: boolean }) => {
            state.naviMount = action.payload
        }
    }
})

export const { setNaviBreadcrumb, setNaviShown, setNaviMount } = sphereTreeSlice.actions
// Other code such as selectors can use the imported `RootState` type
export const selectSphereTree = (state: RootState) => state.sphereNavigation
